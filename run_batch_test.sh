...
echo "batch_size,duration_seconds,tps" > ~/Desktop/Blockchain/results.csv
...
for BATCH in 20 40 80 160 200; do
    ...
    for i in $(seq 1 $BATCH); do
        peer chaincode invoke \
            ...
            -c "{\"function\":\"LogEvent\",\"Args\":[\"REC${BATCH}_${i}\",\"AgencyA\",\"INCIDENT\",\"Test event $i\"]}" \
            > /dev/null 2>&1
    done
    END=$(date +%s%N)
    DURATION=$(echo "scale=3; ($END - $START) / 1000000000" | bc)
    TPS=$(echo "scale=2; $BATCH / $DURATION" | bc)
    echo "$BATCH,$DURATION,$TPS" >> ~/Desktop/Blockchain/results.csv
    echo "Batch $BATCH done: ${DURATION}s, TPS: $TPS"
done

echo "Done! Results saved to ~/Desktop/Blockchain/results.csv"
